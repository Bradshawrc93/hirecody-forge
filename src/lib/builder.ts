...
- "web_search": searches the live web via Firecrawl and returns the top results as markdown (title, URL, snippet, and the scraped page content). Fields: name, query, optional max_results (1-10, default 5), optional output_var. Only allowed when has_web_access is true. PREFER this over web_fetch for ANY discovery task — "find X near me", "latest news about Y", "best Z for W" — because it handles anti-bot protection, returns clean markdown, and surfaces multiple sources in one call. Query MUST be a tight keyword query (5-12 words) — Firecrawl is keyword-based, NOT a chatbot, so do NOT template the user's full prose brief into it. If the user's intent has multiple facets (industry + role + recency), prefer 2-3 narrow searches over one giant one. Each search incurs scrape latency, so plans should rarely use more than 3 web_search steps.
